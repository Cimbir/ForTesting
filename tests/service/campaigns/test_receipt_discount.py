...
def test_should_add_and_get_receipt_discount(
    ...
    receipt_discount_response = receipt_discount_service.add_receipt_discount(
        receipt_discount
    )
...
def test_should_get_all_receipt_discounts(
    receipt_discount_service: ReceiptDiscountService,
) -> None:
...
def test_should_remove_receipt_discount(
    receipt_discount_service: ReceiptDiscountService,
) -> None:
    receipt_discount = ReceiptDiscount(
        id="",
...
def test_should_raise_receipt_discount_not_found_when_getting_non_existent_discount(
    ...
    pytest.raises(
        ReceiptDiscountNotFound, receipt_discount_service.get_receipt_discount, "1"
    )
