...
from finalproject.service.exceptions import ProductDiscountNotFound, ProductNotFound
from finalproject.store.product import ProductRecord


def test_should_add_and_get_product_discount(
    ...
    product_discount_response = product_discount_service.add_product_discount(
        product_discount
    )
...
def test_should_get_all_product_discounts(
    product_discount_service: ProductDiscountService,
) -> None:
...
def test_should_raise_product_discount_not_found_when_getting_non_existent_discount(
    ...
    pytest.raises(
        ProductDiscountNotFound, product_discount_service.get_product_discount, "1"
    )


def test_should_remove_product_discount(
    product_discount_service: ProductDiscountService,
) -> None:
    product_discount_service.product_store.add(ProductRecord("1", "product 1", 1.0))
    product_discount = ProductDiscount(
