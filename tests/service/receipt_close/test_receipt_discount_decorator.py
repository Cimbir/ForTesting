...
def test_should_return_zero_on_empty_receipt(def_rec_close: DefaultReceiptClose) -> None:
...
def test_should_discount_receipt(def_rec_close: DefaultReceiptClose) -> None:
    ...
    assert rec_discount.close(get_receipt([
        ReceiptItem(
            id="1",
            product_id="1",
            quantity=1,
...
def test_should_choose_the_only_discount_that_fits(def_rec_close: DefaultReceiptClose) -> None:
    ...
    rec_discount = ReceiptDiscountDecorator(rec_discount, ReceiptDiscount(
        id="2",
        discount=0.2,
        minimum_total=20.0
    ))
