...
def test_should_return_zero_on_empty_receipt(
    ...
    rec_discount = ReceiptDiscountDecorator(
        def_rec_close, ReceiptDiscount(id="1", discount=0.1, minimum_total=10.0)
    )
...
def test_should_discount_receipt(def_rec_close: ReceiptClose) -> None:
    ...
    assert (
        rec_discount.close(
            get_receipt([ReceiptItem(id="1", product_id="1", quantity=1, price=10.0)])
        ).price
        == 9.0
    )
...
def test_should_not_discount_because_lower_than_main(
    ...
    assert (
        rec_discount.close(
            get_receipt([ReceiptItem(id="1", product_id="1", quantity=1, price=9.0)])
        ).price
        == 9.0
    )
...
def test_should_discount_multiple_products(def_rec_close: ReceiptClose) -> None:
    ...
    assert (
        rec_discount.close(
            get_receipt(
                [
                    ReceiptItem(id="1", product_id="1", quantity=1, price=7.0),
                    ReceiptItem(id="2", product_id="2", quantity=1, price=12.0),
                ]
            )
        ).price
        == (7.0 + 12.0) * 0.9
...
def test_should_not_discount_if_total_lower_than_main(
    ...
    assert (
        rec_discount.close(
            get_receipt(
                [
                    ReceiptItem(id="1", product_id="1", quantity=1, price=4.0),
                    ReceiptItem(id="2", product_id="2", quantity=1, price=5.0),
                ]
            )
        ).price
        == 9.0
...
def test_should_choose_the_only_discount_that_fits(
    ...
    rec_discount = ReceiptDiscountDecorator(
        rec_discount, ReceiptDiscount(id="2", discount=0.2, minimum_total=20.0)
    )
    assert (
        rec_discount.close(
            get_receipt(
                [
                    ReceiptItem(id="1", product_id="1", quantity=1, price=3.0),
...
def test_should_choose_the_best_discount(def_rec_close: ReceiptClose) -> None:
    ...
    assert (
        rec_discount.close(
            get_receipt(
                [
                    ReceiptItem(id="1", product_id="1", quantity=1, price=13.0),
                    ReceiptItem(id="2", product_id="2", quantity=1, price=10.0),
                ]
            )
        ).price
